...
describe("resolveRateLimiter", () => {
  ...
  it("blocks memory in production without explicit override", () => {
    expect(() =>
      resolveRateLimiter({ NODE_ENV: "production", OHM7_RATE_LIMIT_PROVIDER: "memory" }),
  ...
  it("allows memory in production with the explicit escape hatch", () => {
    const r = resolveRateLimiter({
      NODE_ENV: "production",
      OHM7_RATE_LIMIT_PROVIDER: "memory",
  ...
  it("returns a disabled limiter when mode=disabled", async () => {
    const r = resolveRateLimiter({ NODE_ENV: "production", OHM7_RATE_LIMIT_PROVIDER: "disabled" });
  ...
  it("redis throws without REDIS_URL", () => {
    expect(() =>
      resolveRateLimiter({ NODE_ENV: "production", OHM7_RATE_LIMIT_PROVIDER: "redis" }),
    ).toThrow(RateLimitMisconfiguredError);
  });

  it("memory limiter actually enforces the limit", async () => {
    const r = resolveRateLimiter({ NODE_ENV: "test", OHM7_RATE_LIMIT_PROVIDER: "memory" });
